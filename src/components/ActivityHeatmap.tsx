...
export default function ActivityHeatmap() {
  ...
  const flatGrid = grid.reduce((acc, curr) => acc.concat(curr), []);
  const totalSubmissions = flatGrid.reduce((acc, curr) => acc + (curr > 0 ? curr : 0), 0);

  return (
    <div className="bg-[#0b1311]/90 border border-[#1e293b] rounded-3xl p-6 space-y-4 shadow-2xl motion-card">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#1e293b] pb-3">
        <div>
          <h3 className="text-base font-bold text-white flex items-center gap-2">
            <Flame className="w-4 h-4 text-[#f59e0b] fill-[#f59e0b]" aria-hidden="true" />
            Daily Learning & Code Activity
          </h3>
