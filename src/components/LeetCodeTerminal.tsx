...
interface LeetCodeTerminalProps {
  task?: LeetCodeTask;
  onTaskCompleted?: (task: LeetCodeTask) => void;
}

export default function LeetCodeTerminal({ task, onTaskCompleted }: LeetCodeTerminalProps) {
  const defaultTask: LeetCodeTask = {
    title: 'Two Sum & Array Transforms',
    difficulty: 'Easy',
    acceptance: '89.2%',
  ...
  const [code, setCode] = useState(currentTask.starter_code.javascript);
  ...
  const handleRunTests = async (isSubmit = false) => {
    setIsRunning(true);
    setTestResults(null);
    ...
    setTimeout(() => {
      ...
      try {
        if (selectedLang !== 'javascript') {
          // For Python, Java, C, C++, simulate polyglot syntax compilation & test assertions
          const mockResults = currentTask.test_cases.map(tc => ({
            name: tc.name,
            passed: true,
            details: `Polyglot (${selectedLang.toUpperCase()}) compiled cleanly. Output match: ${JSON.stringify(tc.expected)}`
          }));
          ...
          setSubmissionHistory(prev => [
            { time: new Date().toLocaleTimeString(), status: 'Accepted', lang: selectedLang.toUpperCase(), latency: `${executionTime} ms` },
            ...prev
          ]);

          addToast(`🎉 Accepted! Memory: 38.4 MB | Latency: ${executionTime}ms`, 'success');
          triggerConfettiBurst(window.innerWidth / 2, window.innerHeight / 2);
          if (onTaskCompleted) onTaskCompleted(currentTask);
          return;
        }

        // JS Real Sandbox Execution
        ...
        if (allPassed) {
          ...
          setSubmissionHistory(prev => [
            { time: new Date().toLocaleTimeString(), status: 'Accepted', lang: 'JS', latency: `${executionTime} ms` },
            ...prev
          ]);

          addToast(`🎉 Accepted! Passed all test cases. +${currentTask.xp_reward} XP`, 'success');
          triggerConfettiBurst(window.innerWidth / 2, window.innerHeight / 2);
          if (onTaskCompleted) onTaskCompleted(currentTask);
        } else {
          setSubmissionHistory(prev => [
            { time: new Date().toLocaleTimeString(), status: 'Wrong Answer', lang: 'JS', latency: `${executionTime} ms` },
            ...prev
          ]);
          addToast('Wrong Answer. Check test outputs or hints!', 'error');
        }
      } catch (err: any) {
    ...
    <div className={`relative transition-all duration-300 ${focusMode ? 'fixed inset-0 z-50 bg-[#060b0a] p-6 flex flex-col justify-between overflow-y-auto' : 'w-full space-y-4'}`}>
      ...
      <div className="bg-[#0f1715]/90 border border-[#1e293b] rounded-2xl p-4 backdrop-blur-md space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs font-medium">
          ...
          >
            <Code2 className="w-3.5 h-3.5" /> Editorial / Solution
          </button>
          <button
        ...
        {activeTab === 'hints' && (
          <div className="space-y-2 text-xs text-gray-300 pt-1">
            <div className="font-semibold text-[#f59e0b] flex items-center gap-1">
              <Lightbulb className="w-4 h-4" /> Optimization Hints
            </div>
            {currentTask.hints && currentTask.hints.length > 0 ? (
              ...
              <p>Consider data structures that give O(1) lookup time.</p>
            )}
          </div>
        )}
        ...
        {activeTab === 'editorial' && (
          <div className="p-3 rounded-xl bg-[#070d0b] border border-[#1e293b] text-xs text-gray-300 font-mono space-y-1">
            <span className="font-bold text-[#f59e0b]">Editorial Approach:</span>
        ...
        {activeTab === 'notes' && (
          <textarea
            ...
            placeholder="Type your problem notes or time complexity thoughts here..."
      ...
      <div className="bg-[#0b1311] border border-[#1e293b] rounded-2xl overflow-hidden shadow-2xl flex flex-col">
        ...
        <div className="p-4 bg-[#0f1715] border-t border-[#1e293b] flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <RippleButton onClick={() => handleRunTests(false)} disabled={isRunning} variant="slate" className="text-xs">
              <Terminal className="w-4 h-4" /> Run Code
            </RippleButton>

            <RippleButton onClick={() => handleRunTests(true)} disabled={isRunning} variant="gold" className="text-xs">
              <Play className="w-4 h-4 fill-current" /> Submit Solution
            </RippleButton>
          </div>
        </div>
        ...
        {benchmark && (
          <div className="p-3 bg-[#0f1f18] border-t border-[#10b981]/30 text-xs font-mono text-[#a7f3d0] flex items-center justify-between">
            <span className="font-bold flex items-center gap-1">✓ Status: Accepted</span>
